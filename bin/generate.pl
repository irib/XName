...
sub RetrieveRecords {
	...
	$sth1 = dbexecute("SELECT val1,val2,ttl
		FROM dns_record 
...
#  check if error. If error, DO NOT RELOAD
		...
		$error = 0;
		@result=`$CHECKCONF_COMMAND $NAMED_CONF 2>&1`;
		# should be
		# if ( WEXITVALUE($?) ) {
		if ($?>>8) {
			$error = 1;
...
/g;
			...
			if(!sendmail %mail) {
				include($XNAME_HOME . "strings/" . $EMAIL_DEFAULT_LANGUAGE . "/strings.inc");
				print LOG logtimestamp() . " " . $LOG_PREFIX . " : " . 
